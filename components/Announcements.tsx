      ...
      <div className="p-8 max-w-5xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-300">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => handleDelete(selectedItem.id)} className="px-3 py-1.5 border border-red-200 text-red-500 rounded-lg hover:bg-red-50 flex items-center gap-2 text-sm font-medium transition-colors"><Trash2 size={16} /> 刪除</button>
          </div>
        </div>
          ...
          <div className="space-y-4">
            {filteredItems.length > 0 ? (
              filteredItems.map(item => (
                <div key={item.id} onClick={() => setSelectedItem(item)} className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm hover:shadow-md hover:border-[#FF4B7D]/30 transition-all cursor-pointer group relative overflow-hidden">
                  ...
                  <p className="text-gray-500 text-sm line-clamp-2 mb-3">{item.content.replace(/<[^>]*>?/gm, '').substring(0, 150)}...</p>
                  <span className="px-2 py-0.5 bg-gray-100 text-gray-600 text-xs rounded border border-gray-200 font-medium">{CATEGORY_LABELS[item.category]}</span>
                </div>
              ))
            ) : (
